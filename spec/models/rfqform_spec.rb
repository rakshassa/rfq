...
describe Rfqform do
	before do
		...
		@tlx = User.create(name: "testTLX", isTLX: true)
		@vendor = User.create(name: "testVendor1", isTLX: false, vendor_id: 1)
	end
	...
	describe "create search" do
		before { @newsearch_id = @rfqform.create_search }		
	...
	describe "auth quotes" do
		let!(:vendor) do FactoryGirl.create(:vendor) end
		let!(:vendor2) do FactoryGirl.create(:vendor) end
		let!(:rfqpart) do FactoryGirl.create(:rfqpart, rfqform: @rfqform) end
		...
		describe "as tlx" do
			before do set_user(@tlx) end
			subject { @rfqform.auth_quotes }
		
			its(:count) { should eq(2) }
		end
		describe "as vendor" do
			before do set_user(@vendor) end
			subject { @rfqform.auth_quotes }
